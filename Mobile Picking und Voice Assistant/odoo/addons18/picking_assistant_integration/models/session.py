...
class PickingAssistantSession(models.Model):
    ...
    # Odoo 18: declarative models.Constraint existiert erst ab 19 — klassische
    # _sql_constraints verwenden.
    _sql_constraints = [
        ("session_id_unique", "UNIQUE(session_id)", "Session ID must be unique."),
        ("token_hash_unique", "UNIQUE(token_hash)", "Session token hash must be unique."),
    ]
