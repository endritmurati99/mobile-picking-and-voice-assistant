...
| Der Einbettungsabgleich traegt nur INNERHALB einer Bildwelt | `embed/server.py`, `n8n_v2._abgleich_ueber_einbettung` | Lauf 21, 16.09.2026: Fotos mit Gemini erzeugt, **ohne Katalogbild als Vorlage**, nur aus einer Textbeschreibung. Der erwartete Artikel landete auf **Platz 6**, Spitzenwert 0,6087 fuer ein falsches Teil, Abstand 0,0546 — also kein `unsicher`, sondern eine falsche Gewissheit. In den Laeufen 4 bis 20 lag der richtige Artikel bei 0,64 bis 0,93, weil die Meldefotos AUS dem Katalogbild gerechnet wurden und Pose, Licht und Rendering teilten | Die frueheren `match`-Werte belegen, dass der Einbettungsweg innerhalb einer Bildwelt trennt — nicht, dass er ein fremdes Foto einem Katalogbild zuordnet. Der Widerspruchszweig faengt den Fall ab (`review_required`), aber die Achse selbst ist bildweltgebunden |
| Artikelabgleich sucht bei `zu_dicht` auf dem naechsten Foto weiter | `n8n_v2.py`, `_check_article` | Lauf 17, 16.09.2026: Foto 1 `unsicher` bei Abstand 0,0155, Foto 2 `match` bei 0,0362 — **246 ms statt 59,8 s** Textweg, und ein Bildaufruf weniger. Grundlage: alle 61 archivierten Meldefotos gegen den Einbettungsdienst | Nur Foto 1 zaehlt, und ein knappes erstes Foto kostet 22 % der Laufzeit, obwohl das zweite eindeutig ist |
| `EINBETT_KNAPP_SCHWELLE` bleibt bei 0,02 | `embed/server.py:62` | **Neu belegt am 16.09.2026** ueber alle 61 archivierten Meldefotos: unterhalb der Schwelle ist Platz 1 in 5 von 9 Faellen richtig, und die Werte ueberlappen vollstaendig (0,0026 falsch, 0,0027 richtig). Eine niedrigere Schwelle erzeugt drei falsche `mismatch` | Ein falsches `mismatch` weist eine echte Schadensmeldung ab — der teure Fehler. Die Schwelle senkt keine Laufzeit, das tut die Weitersuche auf dem naechsten Foto |
| Soll-Befund des Katalogbilds liegt als Datei neben dem Speicher | `n8n_v2.py`, Volume `backend_cache` | Lauf 18, 16.09.2026: `soll_cache_geladen` nach dem Neustart, Zustandsvergleich **3,9 s statt 22,2 s**, Katalogbildaufruf entfaellt | Jeder Neustart kostet die jeweils erste Meldung je Artikel erneut einen 22-s-Bildaufruf aus ihrem Bildbudget |
