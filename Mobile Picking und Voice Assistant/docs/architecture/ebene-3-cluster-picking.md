...
Stand: 14. August 2026. Bewertet wurde die überarbeitete Darstellung gegen die
PWA-Aufrufe, FastAPI-Cluster-Routen und den Cluster-Service einschließlich
Besitzprüfung, Package-Zuordnung, Move-Line-Buchung und Batch-Abschluss.
...
Die einzelnen Schritte sind im Code, in Tests und seit dem 13. August 2026
auch am laufenden Odoo-19-Stack belegt: Die PWA meldete `Lena Lager` an,
erzeugte `BATCH/OUT/00001` aus zwei Aufträgen und ordnete jedem Auftrag ein
eigenes Ziel-Package zu. Der frühere Fehler zu
`res_users.totp_last_counter` stammte aus einem veralteten CLI-Profil, das
noch die Odoo-18-Datenbank statt `masterfischer_o19` ansprach.

Die visuelle 2-plus-2-Bündelung wurde zusätzlich als ausführbarer Unit-Test und
mit Browser Use bei 360 und 412 Pixel Breite geprüft. Dabei blieb der Stopp bei
vier Stück, während zwei getrennte Confirm-Requests mit jeweils zwei Stück und
dem richtigen Karton gesendet wurden. Der Browser hatte keinen horizontalen
Überlauf. Die vollständigen Cluster-Service-Tests blieben grün.

## Kurz zusammengefasst

1. FastAPI schlägt passende Aufträge vor.
2. Odoo erhält aus den verbliebenen gültigen Aufträgen einen echten Batch und
   einen Karton je Auftrag mit Move-Lines.
3. FastAPI liefert atomare Move-Lines in gemeinsamer Lagerreihenfolge.
4. Die PWA bündelt geeignete ungetrackte Positionen nach Produkt und Quellort;
   der Artikel wird einmal geprüft, danach jeder Zielkarton separat.
5. Jede Aufteilung bleibt an ihren Auftrag, ihre Move-Line und ihren atomaren
   FastAPI-zu-Odoo-Write gebunden.
6. Die PWA sperrt den Abschluss bis alle Aufteilungen erledigt sind; FastAPI
   ruft danach Odoos `action_done` auf, besitzt aber noch die dokumentierten
   Mengen-, Fortschritts- und Nachlesegrenzen.
